...
document.addEventListener('DOMContentLoaded', () => {
  ...
  app.innerHTML = `
    ...
    <main class="min-h-screen pt-20 flex flex-col lg:flex-row bg-[var(--bg-primary)]">
      ...
      <div class="hidden lg:flex lg:w-1/2 relative items-center justify-center overflow-hidden group">
        ...
        <img src="https://images.unsplash.com/photo-1552374196-1ab2a1c593e8?ixlib=rb-4.0.3&auto=format&fit=crop&w=1200&q=80" 
        ...
        <div class="relative z-20 text-center px-12 pb-20 translate-y-8 group-hover:translate-y-0 transition-transform duration-700 ease-out">
          <h2 class="text-5xl lg:text-6xl font-black font-heading tracking-tighter mb-4 text-white leading-tight drop-shadow-lg">Define Your<br>Aesthetic</h2>
          <p class="text-lg text-gray-200 max-w-md mx-auto opacity-0 group-hover:opacity-100 transition-opacity duration-700 delay-300 drop-shadow-md">Unlock early access to our premium collections, exclusive drops, and member-only benefits.</p>
        </div>
      </div>
      ...
      <div class="w-full lg:w-1/2 flex items-center justify-center px-4 sm:px-8 md:px-16 py-12 lg:py-20 relative overflow-hidden bg-[var(--bg-primary)]">
        ...
        <div class="absolute bottom-0 left-0 w-64 md:w-80 h-64 md:h-80 bg-blue-500 rounded-full filter blur-[100px] opacity-[0.08] md:opacity-[0.12] pointer-events-none"></div>
        ...
        <div class="w-full max-w-md relative z-10">
          <div class="bg-[var(--bg-secondary)] border border-[var(--border-color)] rounded-[2rem] p-8 sm:p-10 shadow-xl relative overflow-hidden transition-all hover:shadow-2xl">
            ...
            <div class="text-center mb-10">
              <h1 class="text-4xl font-heading font-black tracking-tight mb-2 bg-clip-text text-transparent bg-gradient-to-r from-[var(--brand-accent)] to-purple-500">Welcome Back</h1>
              <p class="text-[var(--text-secondary)]">Sign in to your MIST.CO account</p>
            </div>
            ...
            <form id="loginForm" class="space-y-6">
              ...
              <div class="relative group">
                <div class="flex justify-between items-center mb-2">
                  <label for="password" class="block text-xs font-bold uppercase tracking-wider text-[var(--text-secondary)]">Password</label>
                  <a href="#" class="text-xs font-medium text-[var(--text-secondary)] hover:text-[var(--brand-accent)] transition-colors">Forgot password?</a>
                </div>
                <div class="relative">
                  <span class="absolute inset-y-0 left-0 flex items-center pl-4 text-[var(--text-muted)] group-focus-within:text-[var(--brand-accent)] transition-colors">
              ...
              <button type="submit" id="submitBtn" class="w-full relative group overflow-hidden bg-[var(--brand-accent)] text-white font-bold text-lg py-3.5 rounded-xl transition-all duration-300 shadow-lg hover:shadow-[0_8px_25px_rgba(109,40,217,0.4)] hover:-translate-y-0.5">
                ...
                <span class="relative z-10 transition-transform duration-300 inline-block group-hover:scale-105">Sign In</span>
              </button>
            </form>
            
            <p class="mt-8 text-center text-[var(--text-secondary)] text-sm font-medium">
              Don't have an account? 
              <a id="registerLink" href="/pages/register.html" class="text-[var(--brand-accent)] hover:text-[var(--brand-accent-hover)] hover:underline transition-colors font-bold">Sign up here</a>
            </p>
          </div>
        </div>
      </div>
  ...
  if (redirect) {
    ...
    if (registerLink) {
      registerLink.href = `/pages/register.html?redirect=${encodeURIComponent(redirect)}`;
    }
  }
